...
> [!TIP]
> U kunt urenstaten gebruiken op een mobiel apparaat. Om dat te doen moet u mogelijk de schakelaar **Nieuwe urenstaatervaring gebruiken** aanzetten op de pagina [Resources instellen](https://businesscentral.dynamics.com/?page=462) .

## <a name="create-time-sheets"></a>Urenstaten maken
...
1. Kies het pictogram ![Lampje dat de functie Vertel me opent.](media/ui-search/search_small.png "Vertel me wat u wilt doen"), voer **Urenstaten maken** in en kies vervolgens de gerelateerde koppeling.

    > [!TIP]
    > U kunt ook de actie **Urenstaten maken** gebruiken op de pagina **Resources** en de pagina **Resourcekaart**.
2. Vul de vereiste velden in. [!INCLUDE[tooltip-inline-tip](includes/tooltip-inline-tip_md.md)]

    > [!NOTE]  
    > Voordat u urenstaten voor een resource maakt, moet u ervoor zorgen dat de velden **Urenstaat gebruiken** en **Gebruikers-id van eigenaar van urenstaat** ervoor zijn opgegeven op de pagina **Resourcekaart**.

    Kies optioneel de actie **Plannen** om op te geven hoe vaak u wilt dat de taak automatisch wordt uitgevoerd. Als u bijvoorbeeld de taak wilt configureren om gedurende vier weken wekelijks te worden uitgevoerd, voert u op de pagina **Een rapport plannen - Urenstaten maken** in het veld **Formule voor volgende uitvoeringsdatum** **4W** in. Ga voor meer informatie over het plannen van rapporten naar [Een rapport plannen voor uitvoering](ui-work-report.md#ScheduleReport)  

U kunt de urenstaten die u maakt, bekijken op de pagina **Urenstaten**. Elke urenstaat bestaat uit een of meer regels die de tijd definiëren die u ter goedkeuring wilt indienen. In de volgende tabel worden de soorten regels beschreven die u aan de urenstaat kunt toevoegen.
...
| Resource | Wordt gebruikt om tijdverbruik te registeren voor een resource. Geef, om de informatie voor de regel te voltooien, een omschrijving van het werk. |
| Service | Gebruik dit om tijdverbruik voor een serviceorder of servicecreditnota te registreren. |

U wilt bijvoorbeeld een urenstaat indienen voor een week waarin u schoonmaakwerkzaamheden hebt uitgevoerd en een vrije dag had voor een medische afspraak. De volgende tabel laat zien hoe u regels aan de urenstaat toevoegt.

| Type | Omschrijving | Code van werksoort | Afwezigheidstypecode |
|--|--|--|--|
| Resource | Werkuren | Schoonmaakkosten |  |
| Afwezigheid | Verlof |  | Gezondheid |
|  | Ik moest dinsdag vrij nemen vanwege een medische afspraak. |  |  |

In dit hypothetische voorbeeld kunt u dan de uren registreren over de relevante dagen voor elke weekdag.  
...
## <a name="copy-project-planning-lines-to-a-time-sheet"></a>Projectplanningsregels kopiëren naar een urenstaat
...
## <a name="define-work-types-and-add-one-to-a-time-sheet"></a>Werksoorten definiëren en er een toevoegen aan een urenstaat
...
4. Vul de rest van de urenstaat in. Zie voor meer informatie het gedeelte [Urenstaatregels invullen en ter goedkeuring verzenden](#fill-in-time-sheet-lines-and-submit-for-approval).  

> [!TIP]
> U kunt soortgelijke stappen volgens om afwezigheidscodes te definiëren.

## <a name="fill-in-time-sheet-lines-and-submit-for-approval"></a>Urenstaatregels invullen en ter goedkeuring verzenden
...
3. Vul de velden indien nodig op een regel in. Voer het aantal uren in dat door de resource op elke dag van de week wordt gebruikt.  
    ...
    > [!TIP]  
    > U kunt de som controleren van de urenstaaturen die zijn ingevoerd in het feitenblok **Overzicht van werkelijk/gebudgetteerd**.  
4. Herhaal stap 3 voor andere werksoorten die de resource uitvoert.  

    Bepaal vervolgens of u alle of geselecteerde regels op de urenstaat wilt indienen.  
    ...
    * Om de urenstaat voor een of meer regels in te dienen kiest u de betreffende regel en kiest u vervolgens de actie **Indienen**.
        ...
        Kies op de inzendingspagina de optie **Alleen geselecteerde regels**. De staat van de regel verandert van **Open** in **Ingediend**.
    * Om de urenstaat voor alle openstaande regels in te dienen kiest u de actie **Indienen** bovenaan de pagina **Urenstaat**.  

        Bevestig dat u alle openstaande regels op de huidige urenstaat wilt indienen.  
...
* De persoon die is opgegeven in het veld **Gebruikers-id van fiatteur van urenstaat** op een resourcekaart kan urenstaten van die resource goedkeuren. Ga voor meer informatie over het instellen van goedkeuring naar [Urenstaten instellen](projects-how-setup-time-sheets.md).
...
Zodra u een urenstaat goedkeurt of weigert, kan deze niet meer worden gewijzigd tenzij deze eerst opnieuw wordt geopend. In de volgende procedure wordt uitgelegd hoe u een goedgekeurde of geweigerde urenstaat opnieuw opent.

## <a name="reopen-a-time-sheet"></a>Een urenstaat opnieuw openen
...
2. Open een urenstaat uit de lijst.  
    ...
    > [!NOTE]  
    > U kunt alleen regels met de status **Goedgekeurd** opnieuw openen. U kunt geen regels met de status **Afgewezen** of regels die zijn geboekt, opnieuw openen.  
...
## <a name="view-and-approve-time-sheets-by-project"></a>Urenstaten per project bekijken en goedkeuren
...
### <a name="approve-or-reject-a-time-sheet-by-project"></a>Een urenstaat per project goedkeuren of weigeren
...
2. Kies de actie **Goedkeuren** en kies vervolgens de actie **Alle verzonden regels** om alle regels goed te keuren of kies de actie **Alleen geselecteerde regel(s)** om alleen de regels goed te keuren die zijn geselecteerd op de pagina **Urenstaat**.
    ...
    > [!NOTE]
    > U kunt alleen urenstaten goedkeuren met de status **Ingediend**.
...
## <a name="post-time-sheet-lines-in-a-resource-journal"></a>Urenstaatregels naar een resourcedagboek boeken
...
3. Vul op de pagina **Resourcedagboekregels voorstellen** de benodigde velden in. [!INCLUDE [tooltip-inline-tip_md](includes/tooltip-inline-tip_md.md)] 
...
## <a name="post-time-sheet-lines-in-a-project-journal"></a>Urenstaatregels naar een projectdagboek boeken
...
## <a name="archive-time-sheets"></a>Urenstaten archiveren
