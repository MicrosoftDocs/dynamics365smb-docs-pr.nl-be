...
- Uw account heeft ten minste de rol [Exchange Administrator](/entra/identity/role-based-access-control/permissions-reference#exchange-administrator] Microsoft 365 
...
1. Kies **Gecentraliseerde implementatie van Outlook-invoegtoepassing** om de begeleide instelling te starten.
1. Bekijk de eerste pagina en kies **Volgende** om de pagina te openen voor het downloaden van de invoegtoepassingen.
1. Selecteer in de kolom **Implementeren** het selectievakje voor de invoegtoepassingen die u wilt implementeren en kies vervolgens **Downloaden en doorgaan**.

    Een bestand met de naam *OutlookAddins.zip* wordt gedownload naar uw apparaat.

1. Op dit punt bent u klaar met het werk dat u moet doen in Business Central, dus u kunt **Gereed** kiezen.
...
1. Ga naar de map waar OutlookAddins.zip is gedownload en pak de bestanden **Contact Insights.xml** en **Document View.xml** uit de .zip uit in een map naar keuze.
...
1. Meld u aan bij het Microsoft 365-beheercentrum en ga naar [Geïntegreerde apps](https://go.microsoft.com/fwlink/?linkid=2163967).

1. Kies **Aangepaste apps uploaden**.
1. Stel op de pagina **Apps uploaden om te implementeren** **Type app** in op **Office-invoegtoepassing**.
1. Selecteer **Manifestbestand (.xml) uploaden vanaf apparaat** > **Bestand kiezen**.
1. Selecteer een van de invoegtoepassingsbestanden **Content Insights.xml** of **Documents.xml** die u eerder hebt uitgepakt.
1. Volg de instructies om gebruikers toe te wijzen en de invoegtoepassing te implementeren.
1. Herhaal stap 9 tot en met 11 voor het andere bestand als u dat wilt.
