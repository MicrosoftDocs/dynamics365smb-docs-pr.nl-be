...
De meeste dagboeken zijn gebaseerd op het algemeen dagboek. U kunt alle transacties verwerken op de pagina  **Algemene dagboeken** . Zie voor meer informatie [Transacties direct naar het grootboek boeken](finance-how-post-transactions-directly.md).  
...
U kunt bijvoorbeeld personeelskosten boeken voor vergoedingen. Zie voor meer informatie [Kosten van werknemers registreren en terugbetalen](finance-how-record-reimburse-employee-expenses.md).
 ...
 [!INCLUDE [prod_short](includes/prod_short.md)] Biedt echter ook dagboeken aan die zijn geoptimaliseerd voor specifieke typen transacties, zoals het  *Betalingsjournaal* voor het registreren van betalingen. Zie voor meer informatie [Betalingen en terugbetalingen vastleggen in het betalingsdagboek](payables-how-post-payments-refunds.md).  
...
## <a name="add-context-to-general-journal-transactions"></a>Context toevoegen aan dagboektransacties

Wanneer u een dagboek maakt, kunt u koppelingen toevoegen die context geven aan de transacties ervan. Wanneer u het dagboek boekt, kopieert [!INCLUDE [prod_short](includes/prod_short.md)] de koppelingen naar het geboekte dagboek en de grootboekposten die het dagboek maakt. Het aanbieden van koppelingen kan het leven van uw auditor bijvoorbeeld gemakkelijker maken. Als u afbeeldingen van uw onkostenbewijzen op de Sharepoint-site van uw bedrijf bewaart, kunt u koppelingen naar de bestanden toevoegen. Wanneer u het dagboek boekt om uw onkosten in te dienen, kan uw auditor snel bij de ontvangstbestanden komen.

## <a name="use-journal-templates-and-batches"></a>Dagboeksjablonen en -batches gebruiken

Er zijn verschillende sjablonen voor diversendagboeken. Elke journaalsjabloon wordt vertegenwoordigd door een speciale pagina met specifieke functies en de velden die nodig zijn om die functies te ondersteunen, zoals de pagina  **Betalingsafstemmingsjournalen**  om bankbetalingen te verwerken en de pagina  **Betalingsjournalen**  om uw leveranciers te betalen of uw werknemers te vergoeden. Zie voor meer informatie [Betalingen doen](payables-make-payments.md) en [Klantbetalingen reconciliëren met het ontvangstendagboek of vanuit klantposten](receivables-how-apply-sales-transactions-manually.md).
...
### <a name="validating-general-journal-batches"></a>Dagboekbatches valideren

U kunt een achtergrondcontrole inschakelen om vertragingen bij het boeken te helpen voorkomen. De controle geeft een melding wanneer een fout in het financiële journaal waaraan u werkt, ervoor zorgt dat u het journaal niet kunt boeken. Op de pagina **Algemene dagboekbatches** kunt u kiezen voor **Controle op achtergrondfouten** om financiële dagboeken, zoals algemene dagboeken of betalingsdagboeken, te laten valideren terwijl u eraan werkt. [!INCLUDE[prod_short](includes/prod_short.md)] 

Wanneer u de validatie inschakelt, toont het feitenblok **Document controleren** of **Dagboekcontrole** problemen op de huidige regel en in de hele batch. Validatie vindt plaats wanneer u een financiële dagboekbatch laadt en wanneer u een andere dagboekregel kiest. De tegel **Totaal aantal problemen** in het feitenblok toont het totale aantal problemen dat [!INCLUDE[prod_short](includes/prod_short.md)] heeft gevonden en u kunt ervoor kiezen om een overzicht van de problemen te openen.
...
Met een periodiek dagboek maakt u de posten die u regelmatig boekt, slechts eenmaal. De rekeningen, dimensies, dimensiewaarden enzovoort worden bijvoorbeeld na het boeken in het dagboek bewaard. Als er wijzigingen nodig zijn, kunt u deze elke keer dat u een bericht plaatst, aanbrengen.

### <a name="recurring-method-field"></a>Veld Methode

Het veld **Periodieke methode** is belangrijk. Het bepaalt hoe het bedrag op de dagboekregel na het boeken wordt behandeld. Wanneer u de regel bijvoorbeeld altijd met hetzelfde bedrag wilt boeken, kunt u het bedrag ongewijzigd laten. Wilt u dezelfde rekeningen en dezelfde tekst gebruiken, maar het bedrag varieert, dan kunt u het bedrag na het boeken verwijderen.

| Aan | Zie |
| --- | --- |
|V Vast|Het bedrag wordt na het boeken bewaard. Regels met bedragen van nul blijven in het journaal staan, maar worden niet geboekt. U kunt het bedrag in een latere uitvoering bijwerken.  |
|V Variabel|Het bedrag wordt na het boeken gewist.|
...
|OR Omgekeerd vaRiabel|Het bedrag op de dagboekregel wordt na het boeken verwijderd en de tegenboeking wordt de volgende dag geboekt.|
|OS Omgekeerd saldo|Het geboekte bedrag op de rekening op de regel wordt verdeeld over de rekeningen die zijn opgegeven voor de regel op de pagina **Verdeelsleutels**. Het saldo op de rekening wordt op nul ingesteld en een tegenrekeningspost wordt de dag erop geboekt.|
...
Als u een bepaalde post wilt boeken op de laatste dag van elke maand, kunt u dit op een van de volgende manieren doen:
...
Met dit veld bepaalt u de datum waarop de regel uiterlijk mag worden geboekt. De regel wordt niet geboekt na deze datum.
...
Als het veld leeg is, wordt de regel steeds geboekt totdat u deze uit het dagboek verwijdert.
...
Op de pagina  **Terugkerende dagboeken** kunt u de actie  **Toewijzingen** kiezen om aan te geven hoe bedragen op de regel van het terugkerende dagboek moeten worden toegewezen aan verschillende rekeningen en dimensies. Een verdeelsleutel fungeert als tegenrekeningregel voor de periodieke dagboekregel.
...
> [!NOTE]
> U kunt geen toewijzingsrekeningen gebruiken in journaalbatches met een goedkeuringsworkflow voor het maken van journaalregels.
...
U betaalt elke maand huur en u voert het huurbedrag dus in op de kasrekening op een periodieke dagboekregel. Op de pagina **Verdeelsleutels** kunt u de dimensie Afdeling gebruiken om de onkosten over verschillende afdelingen te verdelen. Bijvoorbeeld op basis van het aantal vierkante meter dat elke afdeling in beslag neemt. De berekening is gebaseerd op het verdelingspercentage op elke regel. U kunt op verschillende manieren toewijzen:
...
### <a name="calculate-the-reversal-date"></a>De tegenboekingsdatum berekenen
...
Wanneer u dagboekregels maakt waarvan u weet dat u ze waarschijnlijk later opnieuw moet maken, kunt u ze als een standaarddagboek opslaan voordat u het artikeldagboek boekt. Hetzelfde geldt voor artikeldagboeken en diversendagboeken.
...
5. Op de aanvraagpagina **Opslaan als standaardartikeldagboek** definieert u een nieuw of bestaand standaardartikeldagboek waarin de regels moeten worden opgeslagen.
    ...
    Als u een of meer standaardartikeldagboeken hebt en u een van deze dagboeken wilt vervangen door de nieuwe reeks artikeldagboekregels, selecteert u het artikeldagboek in het veld **Code**.
...
De volgende procedure is gebaseerd op de pagina  **Algemene dagboeken**, maar is van toepassing op alle andere dagboeken die op het algemene dagboek zijn gebaseerd, zoals de pagina  **Betalingsdagboeken** .
